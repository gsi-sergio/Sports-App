...
import 'package:the_game_app/core/service_container/index.dart';
...
abstract class RestAPIClient extends GetxService {
  ...
  Future<T?> create<T>({
    ...
    dynamic data,
      ...
      _doRequest(
        () async =>
            _dio.post<T>('${_dio.options.baseUrl}/$resource', data: data, options: await _getOptions()),
      );
...
class SecureAccessRestClient extends RestAPIClient {
  final TokenGetter getter;

  SecureAccessRestClient({required this.getter});

  @override
  Future<Options> _getOptions({ResponseType? responseType}) async {
    final _token = await getter();
    return Options(
      contentType: "application/json; charset=UTF-8",
      responseType: responseType,
      headers: {
